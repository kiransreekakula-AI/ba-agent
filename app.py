...
----------------------------------
Fixed version: API key read at call time, not import time.
Works correctly on Render, Streamlit Cloud, and locally.
...
# ─────────────────────────────────────────
# SECRETS
# Key is read fresh inside every function call
# so Render's env vars are always available
# ─────────────────────────────────────────
try:
...
def get_api_key(key_name):
    """Read secret fresh at call time — Render injects env vars at runtime."""
    # 1. OS environment — Render sets these
    val = os.environ.get(key_name, "").strip()
    if val:
        return val
    # 2. Streamlit secrets panel — Streamlit Cloud
    try:
...
if not st.session_state.authenticated:
    st.markdown("## 🔒 BA Document Agent")
    pwd = st.text_input("Enter password to access", type="password")
    if st.button("Login"):
        app_password = get_api_key("APP_PASSWORD") or "baagent2026"
        if pwd == app_password:
...
def extract_docx(file_bytes, filename):
    ...
    for para in doc.paragraphs:
        ...
        buffer += " " + text
        if n % 5 == 0:
            blocks.append({"text": buffer.strip(), "source": filename, "page": f"Para {n-4}–{n}"})
            buffer = ""
    if buffer.strip():
        blocks.append({"text": buffer.strip(), "source": filename, "page": f"Para {n}"})
    return blocks
...
def search_web(question):
    ...
    try:
        ...
        results = TavilyClient(api_key=tavily_key).search(query=question, max_results=3)
        return [{"text": r.get("content",""), "source": r.get("url","Web"), "location": "Web"}
                for r in results.get("results",[])]
    except:
        return []

def get_answer(question, doc_chunks, web_chunks):
    # Read key fresh at call time — guarantees Render env var is available
    api_key = get_api_key("ANTHROPIC_API_KEY")

    if not api_key:
        return "❌ API key not found. Please check your Render environment variables."
    ...
    resp   = client.messages.create(
        ...
        max_tokens = 1500,
        system     = """You are an expert Business Analyst assistant.
RULES:
...
with st.sidebar:
    ...
    if st.session_state.ingested_files:
        ...
        st.markdown(f'<span class="chunk-count">{st.session_state.total_chunks} chunks in memory</span>',
                    unsafe_allow_html=True)
        st.markdown("")
    ...
    tavily_key = get_api_key("TAVILY_API_KEY")
    use_web    = st.toggle("🌐 Web Search", value=bool(tavily_key),
                           help="Search the web for additional context")

    st.divider()

    # Show question count (rate limit awareness)
    st.markdown(f'<div style="font-size:11px;color:#999;">Questions this session: '
                f'{st.session_state.question_count} / 20</div>', unsafe_allow_html=True)
    ...
    st.markdown("")
    st.markdown('<div style="font-size:11px;color:#999;">BA Document Agent v1.1<br>Powered by Claude + ChromaDB</div>',
                unsafe_allow_html=True)
...
if prompt := st.chat_input("Ask anything about your documents..."):
    ...
    # Rate limit: 20 questions per session
    if st.session_state.question_count >= 20:
        st.warning("⚠️ Session limit of 20 questions reached. Click 'Clear Chat' to start a new session.")
        st.stop()
